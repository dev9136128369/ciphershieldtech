...
const BlogEditor = ({ blogs = [], setBlogs = () => { }, currentUser }) => {
  ...
  const [plainText, setPlainText] = useState(existingBlog?.plainText || '');
  const [contentStyle, setContentStyle] = useState(existingBlog?.contentStyle || {
    formats: [],
    blocks: []
  });
  ...
  const handleContentChange = (content, delta, source, editor) => {
    ...
    // Store the styling information
    const formats = editor.getContents().ops.map(op => op.attributes || {});
    const blocks = editor.getContents().ops.map(op => ({
      insert: op.insert,
